...
class MemoryManager {
    ...
    allocateMemory(processName, size) {
        console.log(`Alocando memória para o processo ${processName} - Tamanho: ${size} bytes`
        // Implementação do algoritmo de alocação de memória
        );
        //Encontre o primeiro bloco que se ajusta ao processo
        const blockIndex = this.memoryBlocks.findIndex((block) => block.size >= size);
        if (blockIndex !== -1) {
            ...
            console.log(`Memória alocada para o processo ${processName} - Início: ${selectedBlock.start}, Tamanho: ${size} bytes`);
            selectedBlock.processName = processName;
        }
        else {
            console.log(`Falha na alocação de memória para o processo ${processName} - Não há memória disponível suficiente`);
        }
    }
    allocateMemoryBestFit(processName, size) {
        console.log(`Alocando memória para o processo ${processName} - Tamanho: ${size} bytes`);
    ...
    allocateMemoryWorstFit(processName, size) {
        console.log(`Alocando memória para o processo ${processName} - Tamanho: ${size} bytes`);
    ...
    listMemoryBlocks() {
        ...
        const memoryMap = Array(this.totalMemory).fill(null);
